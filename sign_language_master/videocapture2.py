...
def video_start(tuResolution =(320, 240), nFramePerSecond = 30):
    # Returns videocapture object/stream
    #Parameters:
        #device: 0 for the primary webcam, 1 for attached webcam
    
    yoann= "/Users/imacoda10/Desktop/sign-language qui marche/audrey/nice.mp4"
	
    # try to open webcam device
    oStream = cv2.VideoCapture(yoann)
    ...
    #print("Initialized video device, with resolution %s and target frame rate %d" % \
       # (str(tuResolution), nFramePerSecond))
...
def unittest_opticalflow_fromcamera():
    ...
    while True:
        ...
        if key == ord(' '):
            ...
            arFrames = images_crop(arFrames, 124, 124)
            timer.start()
